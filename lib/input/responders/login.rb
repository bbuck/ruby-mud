  ...
  parse_input_with /\A(y|yes|n|no)\Z/i do |conn, input|
    if conn.internal_state.is_a?(Hash) && conn.internal_state[:confirm_name]
      if input =~ /y|yes/i
        ...
        conn.send_text("Welcome to the Laeron, please enter your character's name or type \"new\"")
      end
    end
  end
  ...
  parse_input_with /(.+)/i do |conn, input|
    ...
    if conn.internal_state == :create_name
      if Player.with_username(input).count > 0 #Player.where("lower(username) = ?", input.downcase).count > 0
        conn.send_text("\nWe're sorry but a player has already taken the name \"[f:white:b]#{name.capitalize}[f:yellow:b].\" Please choose another name.")
      else
      ...
      if conn.internal_state.has_key?(:confirm_password)
        if input == conn.internal_state[:confirm_password]
          ...
          if player.save
            ...
            conn.send_text(player.room.display_text(player))
      ...
      # --- Reenter your Password ---
        ...
        conn.send_text("Please reenter your password:#{ANSI::hidden}")
      ...
      if conn.internal_state.is_a?(Player)
        if conn.internal_state.password == input
          ...
          conn.send_text("\nYou have successfully connected to Laeron!")
          conn.send_text(conn.player.room.display_text(conn.player))
        else
          conn.send_text("\n[f:red:b]The password entered was not correct.")
          conn.internal_state = nil
        end
