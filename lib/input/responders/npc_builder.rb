class NpcBuilderResponder < InputResponder
  # --- Template Helpers -----------------------------------------------------
...
+-----------------------------------------------------------------------------+
[f:green]
= Room ##{editing_npc.id} - [f:cyan:b]#{editing_npc.name}
...
Enter Option >>
  ...
  parse_input_with(/\A2\z/) do
    create_responder(EditorResponder).open_editor(editing_npc, :description, allow_colors: true) do
      create_responder(NpcBuilderResponder).send_npc_builder_menu
    end
  end

  parse_input_with(/\A3\z/) do
    create_responder(EditorResponder).open_editor(editing_npc, :script, syntax: true) do
      create_responder(NpcBuilderResponder).send_npc_builder_menu
    end
  end
...
InputManager.add_responder(:npc_builder, NpcBuilderResponder)
