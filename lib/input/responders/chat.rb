  ...
  module Responder
    class Chat < Base
      ...
      parse_input_with(/\Asay (.+)\z/) do |message|
        ...
        send_no_prompt(Game::ChannelFormatter.format(:say_from, {"%M" => message}))
        chan_text = Game::ChannelFormatter.format(:say_to, {"%N" => player.username, "%M" => message})
      ...
      parse_input_with(/\Axme (.+)\z/, /\Axpost (.+)\z/) do |message|
        message = message.purge_colors
        current_room.transmit(Game::ChannelFormatter.format(:xpost, {"%M" => message}))
      end

      parse_input_with(/\Ame (.+)\z/, /\Apost (.+)\z/) do |message|
        message = message.purge_colors
        current_room.transmit(Game::ChannelFormatter.format(:post, {"%N" => player.username, "%M" => message}))
      end

      parse_input_with(/\Aooc (.+)\z/) do |message|
        message = message.purge_colors
        current_room.transmit(Game::ChannelFormatter.format(:ooc, {"%N" => player.username, "%M" => message}))
      end

      parse_input_with(/\Ageneral (.+)\z/) do |message|
        message = message.purge_colors
        send_to_everyone(Game::ChannelFormatter.format(:general, {"%N" => player.display_name, "%M" => message}))
      end

      parse_input_with(/\Atrade (.+)\z/) do |message|
        message = message.purge_colors
        send_to_everyone(Game::ChannelFormatter.format(:trade, {"%N" => player.display_name, "%M" => message}))
      end

      parse_input_with(/\Anewb (.+)\z/) do |message|
        message = message.purge_colors
        send_to_everyone(Game::ChannelFormatter.format(:newbie, {"%N" => player.display_name, "%M" => message}))
      end
        ...
        if other_player.count > 0
          ...
          if other_player.online?
            Player.connections[other_player.id].each do |other_conn|
              other_conn.send_text(Game::ChannelFormatter.format(:tell_to, {"%M" => player.username, "%M" => message}))
            end
            send_no_prompt(Game::ChannelFormatter.format(:tell_from, {"%N" => other_player.username, "%M" => message}))
          else
      ...
      parse_input_with(/\Aserver (.+)\z/) do |message|
        if player.can_administrate?
          send_to_everyone(Game::ChannelFormatter.format(:server, {"%M" => message}))
        else
          send_not_authorized
        end
      end
